    ...
    <div className="rounded-3xl border border-zinc-200 bg-white p-4 shadow-sm">
      ...
      <div className="mb-3 rounded-2xl bg-zinc-950 p-3 font-mono text-xs text-emerald-300">
        ...
        <div className="flex gap-2">
          ...
          />
          <button onClick={execute} className="rounded-xl bg-emerald-600 px-4 py-2 font-medium text-white hover:bg-emerald-700">
            Отправить
          </button>
        </div>
      </div>
      <div className="flex flex-wrap gap-2 text-xs">
        {QUICK_COMMANDS.map((command) => (
          <button
            key={command}
            onClick={() => { setCliValue(command); onExecute(command); }}
