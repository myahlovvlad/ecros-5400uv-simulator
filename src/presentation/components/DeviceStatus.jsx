...
export function DeviceMetricsCard({ device, onReset }) {
  const handleReset = () => {
    if (!window.confirm("Выполнить полный сброс состояния прибора?")) return;
    onReset();
  };
  ...
  return (
    <div className="rounded-3xl border border-zinc-200 bg-white p-4 shadow-sm">
      <div className="mb-3 flex items-center justify-between">
        ...
        <button onClick={handleReset} className="rounded-xl border border-rose-300 bg-rose-50 px-3 py-1 text-sm font-medium text-rose-700 hover:bg-rose-100">
          Полный сброс
        </button>
      </div>
...
export function VirtualSampleCard({
  device,
  onSampleChange,
  onToggleD2Lamp,
  onToggleWLamp,
    ...
    <div className="rounded-3xl border border-zinc-200 bg-white p-4 shadow-sm">
      ...
      <div className="space-y-3 text-sm">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <label className="rounded-2xl bg-zinc-50 p-3">
            <div className="mb-1 text-zinc-500">W-лампа</div>
